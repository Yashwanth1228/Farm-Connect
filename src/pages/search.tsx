...
//   setFetchedequipment(JSON.parse(router.query.items as string)) ;

  console.log("the fetched equipment is ", fetchedequipment) ;
  ...
  const filteredEquipments = fetchedequipment.filter((item: any) => {
    const data = item._source;
  
    const priceMatch = Number(data.price) <= price;
  
    const typeMatch =
      selectedTypes.length === 0 ||
      selectedTypes.includes(String(data.type));
  
    const availabilityMatch =
      avalibility === "all" || data.available === true;
      ...
      <Main>
        <Layout>
          ...
          <Content>
            ...
            <Grid>
              {filteredEquipments.map((item , i ) : any => {
                ...
                return (
                    ...
                    {/* <CardImage src="https://lh3.googleusercontent.com/aida-public/AB6AXuB57hfn4Ahq_W_1LqXo-yOLp7d6oLiiMK-7CFsMcmVb0S7SXUwVCKe1xanxe3aEWKr1psDV4ATLB2NQtyPloF8YEYhx8weIgnwuRsj58eg9oLrY5QhpTAdmfKHa4LK-RSdIEupWqHdXgMQ-mFKGil0bC1b5EwSbj1smxA03VSZDk4s3LZjqWdLxPHAyvlmxbuqM6CUfetMzs9hvb7MCdr6glsV73txnBCpoisunY7dhmTmY-5sgYmtI-T0k9gNFYiBku3M-dcaIqcw" /> */}
                    <CardImage src={String(data.images[0])} />
  
                    <CardBody>
                      <CardTitle>{String(data.name)}</CardTitle>
                      <Desc>Type: {String(data.type)}</Desc>
  
                      <CardFooter>
                        <Price>₹{Number(data.price)}/day</Price>
                        <Button
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/equipments/${Number(item.id)}`);
